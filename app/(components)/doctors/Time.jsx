...
function Time({ selectedDate, onHourSelect, doctorId }) {
  ...
  const { data: bookings, loading: bookingsLoading } = useFetchData(
    `${BASE_URL}/bookings/bookings`
  );
  const { data: doctor, loading: doctorLoading } = useFetchData(
  ...
  const availableHours = (() => {
    ...
    const isOnLeave = doctor?.leavePeriods?.some((period) => {
      ...
      return (
        selectedDateString >= format(startLeave, "yyyy-MM-dd") &&
        selectedDateString <= format(endLeave, "yyyy-MM-dd")
      );
    });
    ...
    const workSchedule = doctor?.workSchedule?.find(
    ...
    return hours.filter((hour) => {
      const hourDateTimeString = `${format(
        selectedDate,
        "yyyy-MM-dd"
      )}T${hour}`;
      ...
      const isPatientBooked = bookings.some((booking) => {
        ...
        const bookingDateTimeLocalString = `${format(
          bookingDateLocal,
          "yyyy-MM-dd"
        )}T${booking.appointmentTime}`;
        const bookingDateTime = new Date(bookingDateTimeLocalString);

        const bookingEndDateTime = addMinutes(
          bookingDateTime,
          consultationDuration
        );

        return (
          booking.doctor?._id.toString() === doctorId.toString() &&
          hourDateTime >= bookingDateTime &&
          hourDateTime < bookingEndDateTime
        );
      });
  ...
  if (bookingsLoading || doctorLoading) {
    return (
